...
module.exports = async (oldUser, newUser) => {
    ...
    if(oldUser.username.includes(Settings.Tag.Symbol) && !newUser.username.includes(Settings.Tag.Symbol)){ // Tagdan ayrılma durumu :D
        ...
        if(channel) channel.send(`${oldUser}(${oldUser.id}) kişisi \`${Settings.Tag.Symbol}\` Tagımızı çıkardı. **Tagımızı çıkardığı için <@&796318060415680512> Rolünü ondan geri aldım.** **\n  <a:vanhelsing_tac:700310159834087464> **Tagımız : ⵢ**`);
    }
        ...
        if(channel) channel.send(`${oldUser}(${oldUser.id}) adlı kişi \`${Settings.Tag.Symbol}\` Tagımızı aldı, Tagımızı aldığı için <@&796318060415680512> Rolünü ona bahşettim. \n <a:vanhelsing_tac:700310159834087464> Tagımız : ⵢ`);
    }
}
